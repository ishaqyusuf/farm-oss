...
- 2026-03-29: Implemented real authentication. `jose` JWT (HS256, 30-day expiry) replaces demo token in `createTRPCContext`. `apps/api/src/lib/auth.ts` provides `signToken`, `verifyToken`, `hashPassword`, `verifyPassword` (Bun.password argon2id). `auth.signIn` looks up user by 6-digit userId, verifies password, issues JWT. `auth.session` is now a `protectedProcedure` that re-fetches live user from DB. Expo tRPC client attaches `Authorization: Bearer <token>` header on every request via async `headers` callback. Seed updated with 3 dev users (owner/manager/worker, all password `demo1234`) and FarmMember assignment for worker. `JWT_SECRET` env var added to `.env.example`.
- 2026-03-29: Added flock batch selector to mobile app. Created `FarmProvider` context (`apps/expo/src/providers/farm-provider.tsx`) that fetches farms and active batches, auto-selects the first farm and first active batch, and exposes `useFarm()` hook. Wired into `_layout.tsx` inside `AuthProvider`. All screens (record, cages, expenses, sales, history) and `use-dashboard-data` hook now use `useFarm()` for real farmId/flockBatchId — `PLACEHOLDER_FARM_ID` and `PLACEHOLDER_FLOCK_BATCH_ID` constants removed from all screens. Home screen displays a tappable farm/batch selector card that opens a bottom-sheet modal for switching. Fixed `SCREEN_ROLES` in `RoleGuard.tsx`: renamed `"staff"` → `"worker"` to match current role system.
